...
resDir = [CurrDir '\Results\Training\' SubjID '\'];
if ~isdir(resDir)
    mkdir(resDir);
end

if exist([resDir SubjID '_Sess' num2str(SessID) '_motionTrain_run' num2str(RunID) '.mat'],'file')
    ShowCursor;
    Screen('CloseAll');
    reset_test_gamma;
    warning on;
...
elseif SessID == 1 && RunID ~= 1
    previous_results = load([CurrDir '\Results\Training\' SubjID '\' SubjID '_Sess' num2str(SessID) '_motionTrain_run' num2str(RunID-1) '.mat']);
    Param.Staircase.AngleDelta = previous_results.threshold_value;
    elseif SessID ~= 1 && RunID == 1
    previous_results = load([CurrDir '\Results\Training\' SubjID '\' SubjID '_Sess' num2str(SessID-1) '_motionTrain_run' num2str(10) '.mat']);
    Param.Staircase.AngleDelta = previous_results.threshold_value;
elseif SessID ~= 1 && RunID ~= 1
    previous_results = load([CurrDir '\Results\Training\' SubjID '\' SubjID '_Sess' num2str(SessID) '_motionTrain_run' num2str(RunID-1) '.mat']);
    Param.Staircase.AngleDelta = previous_results.threshold_value;
...
if IfEyelink==0
    ...
    mainfilename = [num2str(dayNum) '_' num2str(runNum)];
    dummymode = IfEyelink; 
...
%% Go!
    ...
    for frame_i = 1:Param.RDK.NumFrames
        ...
        Screen('DrawLines', wnd, Param.Fixation.CrossLoc, Param.Fixation.CrossWidth, Param.Fixation.CrossColor, [], 1);
        if frame_i == 1
            vbl = Screen('Flip',wnd,vbl+Param.trainTrial.Delay2);
    ...
    %% Feedback
    % test
    Screen('FillOval',wnd,yellow,[curr_loc(2,1)-Param.SpatialDot.DotSize, curr_loc(2,2)-Param.SpatialDot.DotSize,curr_loc(2,1)+Param.SpatialDot.DotSize,curr_loc(2,2)+Param.SpatialDot.DotSize]);
    Screen('DrawLine',wnd,yellow,Param.Settings.ScrnResolution(3)/2, Param.Settings.ScrnResolution(4)/2,curr_loc(2,1), curr_loc(2,2),1.5);
    %sample
    Screen('FillOval',wnd,black,[curr_loc(1,1)-Param.SpatialDot.DotSize, curr_loc(1,2)-Param.SpatialDot.DotSize,curr_loc(1,1)+Param.SpatialDot.DotSize,curr_loc(1,2)+Param.SpatialDot.DotSize]);
    Screen('DrawLine',wnd,black,Param.Settings.ScrnResolution(3)/2,Param.Settings.ScrnResolution(4)/2,curr_loc(1,1), curr_loc(1,2),1.5);
    ...
    while (GetSecs - timePoints(trial_i,1) < Param.trainTrial.Duration_Beh2)
